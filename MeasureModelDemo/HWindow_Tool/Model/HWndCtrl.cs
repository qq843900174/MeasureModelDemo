...
namespace ViewWindow.Model
{
    ...
    /// <summary>
    /// 这个类用作HWindow的封装类，HWndCtrl负责可视化。
    /// 您可以使用GUI组件输入或使用鼠标移动和缩放可见图像部分。
    /// HWndCtrl类使用图形堆栈来管理显示的图标图像。每个图像都链接到一个图形内容，该内容决定如何绘制对象。
    /// 可以通过调用changeGraphicSettings()更改内容。
    /// 图形模式由GraphicsContext类定义，并映射HDevelop中大多数dev_set_*算子。
    /// </summary>
    public class HWndCtrl
    {
        ...
        //常量描述用于向异常错误发送信号的委托信息
        public const int ERR_READING_IMG = 32;
        //常量描述用于当定义图形内容时，向异常错误发送信号的委托信息
        public const int ERR_DEFINING_GC = 33;

        //可放入图形堆栈而不丢失的最大HObject数，对于每个附加的对象，将从堆栈移除第一个条目。
        //原始值为50 实际上2都可以,因这里只是存储背景图片
